...
fn json_output_runs_without_error() {
    ...
    let result = super::run(&cfg, OutputMode::Json, 20, 50, 4, None);
    assert!(result.is_ok());
}
...
fn run_on_files_with_smell() {
    ...
    let result = super::run_on_files(&[file], OutputMode::Table, 20, 50, 4, None);
    assert!(result.is_ok());
}

#[test]
fn run_on_files_empty_list() {
    let result = super::run_on_files(&[], OutputMode::Table, 20, 50, 4, None);
    assert!(result.is_ok());
}
...
fn run_on_files_json_empty() {
    let result = super::run_on_files(&[], OutputMode::Json, 20, 50, 4, None);
    assert!(result.is_ok());
}
...
fn run_on_files_nonexistent_path_skips() {
    ...
    let result = super::run_on_files(&paths, OutputMode::Table, 20, 50, 4, None);
    assert!(
        result.is_ok(),
...
fn run_on_files_github_format_with_smells() {
    ...
    let result = super::run_on_files(&[file], OutputMode::Table, 20, 50, 4, Some("github"));
...
fn run_on_files_json_format_via_format_param() {
    ...
    let result = super::run_on_files(&[file], OutputMode::Table, 20, 50, 4, Some("json"));
...
fn run_on_files_empty_json_format() {
    ...
    let result = super::run_on_files(&[], OutputMode::Table, 20, 50, 4, Some("json"));
    assert!(result.is_ok());
}
...
fn run_github_format_with_smells() {
    ...
    let result = super::run(&cfg, OutputMode::Table, 20, 50, 4, Some("github"));
