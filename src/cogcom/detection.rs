...
//!
//! Detects function boundaries using either brace-scoped or indent-scoped
//! languages, then delegates to the cognitive complexity analyzer for scoring.
//! Reuses the same detection heuristics as cyclomatic complexity.
...
fn is_function_declaration(trimmed: &str, markers: &CognitiveMarkers) -> bool {
...
/// Heuristic for C/C++/Java/C# function detection.
...
/// Collect all lines belonging to a brace-scoped function body.
...
/// Brace-scoped function detection and cognitive complexity computation.
fn detect_brace_scoped(
    ...
    while i < code_lines.len() {
        ...
        if is_function_declaration(trimmed, markers) {
            ...
            let complexity = count_cognitive_for_lines(&func_code_lines, markers);
...
/// Indent-scoped function detection and cognitive complexity computation.
